...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->button4->Text = L"добавить меропри€тие";
			...
			this->Text = L"часова€ настройка дн€";
			...
			case 1: {
				...
				if (found) {
					...
					if (found_hour) {
						...
						MessageBox::Show("не правильно введено врем€");
						return;
					}
				}
				else {
				...
				if (found) {
					...
					if (found_hour) {
						for (int hour = choice_hour; hour < 19; hour++) {
							if (Convert::ToString(dataGridView1->Rows[num]->Cells[hour]->Value) == "") {
								dataGridView1->Rows[num]->Cells[hour]->Style->BackColor = Color::Silver;
								dataGridView1->Rows[num]->Cells[hour]->Value = "другое до конца дн€";
							}
						}
					}
					else {
